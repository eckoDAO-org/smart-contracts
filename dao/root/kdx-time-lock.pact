...
(module time-lock GOVERNANCE
  ...
  (defcap GOVERNANCE ()
    (enforce-guard
      (keyset-ref-guard 'time-lock-test-keyset)))
  ...
  (defconst KDX_BANK:string 'timelock-test-reward-bank)
  ...
  (defconst START_TIME:time (time "2022-02-23T23:59:00Z"))

  (defconst END_TIME:time (time "2022-03-23T23:59:00Z"))

  (defun kdx-bank-guard:guard () (create-module-guard "kdx-test-admin"))

  (defun init ()
    (dummy-token.create-account KDX_BANK (kdx-bank-guard))
    (fake-token.create-account KDX_BANK (kdx-bank-guard))
  )
    ...
    (let*
      ( (lockup-length (read lockup-options lockup-time))
        ...
        (original-reservations (filter
          (compose (at 'status) (= free.priv-sale.STATUS_APPROVED))
          (free.priv-sale.read-reservations reservation-account)))
        ...
        (kdx-return (floor (* kdx-allocated kdx-return-fraction) (fake-token.precision)))
  ...
  (defun claim-return:object{return-amount} (reservation-account:string)
    (let*
      ...
      (bind lockup-record
        ...
        ;; Payout KDX
        (install-capability (fake-token.TRANSFER KDX_BANK payout-account kdx-available))
        (fake-token.transfer-create KDX_BANK payout-account payout-guard kdx-available)
      ...
      (if (< when lockup-start) { 'kda: 0.0, 'kdx: 0.0 }
        (if (> when lockup-end) { 'kda: kda-total-return, 'kdx: kdx-total-return }
          (let*
            ...
            , 'kdx: (floor (* lockup-progress-fraction kdx-total-return) (fake-token.precision)) }
          )
        )
      ))
    )
