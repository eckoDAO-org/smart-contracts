(module tokens-test-fungible G
  ...
  (defun transfer:string
    ...
    (with-capability (TRANSFER sender receiver amount)
      ...
      (with-capability
        ...
        (kaddex.tokens.transfer TOKEN sender receiver amount))
    )
  ...
  (defun get-balance:decimal
    ( account:string )
    (kaddex.tokens.get-balance TOKEN account)
  )
  ...
  (defun details:object{fungible-v2.account-details}
    ...
    (remove "token"
      (+ { 'guard: (at 'guard (read entries account))}
        (kaddex.tokens.details TOKEN account)))
  )

  (defun precision:integer ()
    (kaddex.tokens.precision TOKEN)
  )

  (defun enforce-unit:bool
    ( amount:decimal )
    (kaddex.tokens.enforce-unit TOKEN amount)
  )
  ...
  (defun create-account:string
    ...
    (kaddex.tokens.create-account TOKEN account (delegate-guard account 0))
  )
  ...
  (defun rotate:string
    ...
    (let*
      ...
      (with-capability (DELEGATE account v)
        (kaddex.tokens.rotate TOKEN account
  ...
  (defun test-fund
    ...
    (kaddex.tokens.credit TOKEN account (delegate-guard account 0) amount)
    true
  )
